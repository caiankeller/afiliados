...
function Product({ params }: { params: { name: string } }) {
  ...
  const parseDate = (dateString: string) => {
    ...
    const parsedDate = new Date(dateString).toLocaleDateString(
      "en-US",
      options
    );
    return parsedDate;
      ...
      <div className="bg-neutral-200 p-2 rounded">
        <div className="flex flex-col justify-between gap-2">
          <div className="flex justify-between">
            ...
            <div className="flex flex-col items-end gap-1 font-normal text-right text-sm">
              <Link
                href="/"
                className="px-3 py-1 bg-rose-500 rounded flex gap-2 items-center font-bold text-sm text-neutral-900  hover:outline outline-1 outline-black hover:duration-150"
              >
                Back
              </Link>
            </div>
          ...
          <div className="text-neutral-900 text-sm ">
            total by product{" "}
            <span className="p-0 px-2 bg-green-500 rounded font-bold">
        ...
        {isLoading ? (
          ...
          product && (
            <div className="mt-4">
              <h4 className="text-neutral-900  text-sm font-bold">
                    ...
                    >
                      <div className="flex justify-between items-center gap-1">
                        <h5 className="text-sm">
                          ...
                          {seller.transactions.length} transactions)
                        </h5>
                      ...
                      <div className="bg-neutral-200 p-2 gap-1 rounded text-neutral-900 mt-3 overflow-y-auto">
                        {seller.transactions.map((transaction, index) => {
                          return (
                            <div
                              key={index}
                              className="flex justify-between odd:bg-neutral-300"
                            >
                              <div className="w-1/6 font-bold">
                                {(transaction.priceTag / 100).toLocaleString(
                                  "pt-BR",
                                  {
                                    style: "currency",
                                    currency: "BRL",
